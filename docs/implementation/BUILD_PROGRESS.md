...
**Current Phase**: Phase 6 (Fan Memory + Scoring) + Phase 7 (AI Gateway) validated by owner. Fan dossier UI (profile fields, tags, human notes) added on top, owner-requested after reviewing a competitor's fan panel. Awaiting owner to apply `0007` and validate.
...
- **Fan dossier UI, owner-requested after comparing against a competitor (MyFeed)**: `0007_fan_profile_tags_notes.sql` — dedicated `fans` columns (birthday, location, income_amount/frequency, subscription_status, source, per owner's choice of structured fields over freeform AI memory), `tags`/`fan_tags` (agency-scoped "Listes", many-to-many), `fan_notes` (spec 8.30 — human notes, never touched by the AI). New `FanProfileCard` component on `/inbox/[id]` (third column: Valeur Fan — total spent/purchase count computed from real purchase messages, not fabricated; Fan Flow stage badge — New/Connaissance/Prêt/Spender, derived from that real data + the existing purchase-intent score, `src/lib/fans/fanFlow.ts`; editable Profil/Abonnement; Listes; Notes). `/inbox` list now shows each conversation's tags and supports filtering by tag via `?tag=`. `next build` verified clean.

**In Progress**: Owner needs to (1) run `0007_fan_profile_tags_notes.sql` in Supabase, (2) validate the new third column on `/inbox/[id]` (edit profile fields, add a tag, add a note, confirm Valeur Fan/Fan Flow look right), (3) validate tag filtering on `/inbox`. Phase 1's remaining app-code items (`authorize()` middleware, structured logging, tenant-isolation automated test) remain intentionally deferred until a feature actually needs them.
